...
import {
    Table, Form, Input, Select, Button, Popconfirm,
    Switch, Tag, Modal, message, Tooltip, Badge, Space
} from 'antd';
import {
    FiPlus, FiEdit2, FiTrash2, FiSearch, FiDroplet, FiX, FiCheck
} from 'react-icons/fi';
import {
    useGetTanksQuery,
...
export function TankManagement() {
    ...
    // ── API hooks ──────────────────────────────────────────────────────────────
    const { data: apiData, isLoading } = useGetTanksQuery({ page: 1, limit: 1000 });
    const [createTank, { isLoading: isCreating }] = useCreateTankMutation();
    const [updateTank, { isLoading: isUpdating }] = useUpdateTankMutation();
    const [deleteTank] = useDeleteTankMutation();

    const tanks = Array.isArray(apiData?.data)
        ? apiData.data
    ...
    async function handleAddOk() {
        try {
            ...
            await createTank({ ...values, is_active: values.is_active ?? true }).unwrap();
    ...
    async function handleEditOk() {
        try {
            ...
            await updateTank({ id: editingRecord.id, ...values }).unwrap();
    ...
    async function handleDelete(record) {
        try {
            await deleteTank(record.id).unwrap();
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (active) =>
                active ? (
                    <Tag
                        icon={<FiCheck size={11} style={{ marginRight: 4 }} />}
                        style={{
                    ...
                    <Tag
                        icon={<FiX size={11} style={{ marginRight: 4 }} />}
                        style={{
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex gap-3 flex-wrap">
                ...
                <SummaryBadge
                    label="Active"
                    value={tanks.filter(t => t.is_active).length}
                    color="#16a34a"
                    bg="#f0fdf4"
                />
                <SummaryBadge
            ...
            <Table
                ...
                rowClassName={(_, idx) =>
                    idx % 2 === 1 ? 'editable-row even-row' : 'editable-row'
                }
            ...
            {/* ── Edit Modal ──────────────────────────────────────────────────── */}
            <TankFormModal
                title={`Edit Tank — ${editingRecord?.name ?? ''}`}
                open={isEditModalOpen}
                form={editForm}
                onOk={handleEditOk}
                onCancel={() => { setIsEditModalOpen(false); setEditingRecord(null); }}
                okText="Save Changes"
