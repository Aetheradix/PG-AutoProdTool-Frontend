...
const MatrixGrid = ({ rows }) => {
  ...
  const { sources, targets, matrix } = useMemo(() => {
    ...
    rows.forEach(r => {
      ...
      const type = String(r.washout_type || "").toUpperCase().trim();
      if (!src || !tgt) return;
      srcSet.add(src);
      tgtSet.add(tgt);
      if (!mat[src]) mat[src] = {};
  ...
  if (!sources.length)
    return <div className="flex justify-center py-16"><Empty description="No matrix data available" /></div>;
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-4 text-xs text-slate-500 font-medium">
          <span><b className="text-slate-700">{filteredSources.length}</b> sources</span>
          <span><b className="text-slate-700">{targets.length}</b> targets</span>
          <span><b className="text-slate-700">{rows.length}</b> rules</span>
        </div>
      </div>
      ...
      <div className="overflow-auto rounded-lg border border-slate-200 shadow-sm max-h-[65vh]">
        <table className="border-collapse text-xs font-mono min-w-full">
          <thead className="sticky top-0 z-10">
            <tr>
              <th className="sticky left-0 z-20 bg-[#002060] text-white px-3 py-2 text-center font-black text-[10px] tracking-widest uppercase whitespace-nowrap" style={{ minWidth: 130 }}>
                SRC \ TGT
              </th>
              {targets.map(tgt => (
                <th key={tgt} className="bg-[#002060] text-white px-2 py-2 font-bold text-[10px] whitespace-nowrap text-center border-l border-blue-900/40" style={{ minWidth: 70 }}>
                  <Tooltip title={tgt}>
            ...
            {filteredSources.map((src, ri) => (
              <tr key={src} className={ri % 2 === 0 ? "bg-white" : "bg-slate-50/60"}>
                ...
                {targets.map(tgt => {
                  ...
                  return (
                    <td key={tgt} className="border-b border-l border-slate-100 px-1 py-1 text-center" style={{ backgroundColor: isSelf ? "#f1f5f9" : (raw ? cfg.bg : "transparent") }}>
                      {isSelf ? (
                        <span className="text-slate-300 font-black">-</span>
                      ) : raw ? (
                        <Tooltip title={`${src} to ${tgt}: ${cfg.label}${cfg.minutes !== null ? ` (${cfg.minutes}min)` : ""}`}>
                          <span className="inline-block px-1.5 py-0.5 rounded-sm font-black text-[9px] tracking-wide leading-tight cursor-default" style={{ color: cfg.color, border: `1px solid ${cfg.border}`, backgroundColor: cfg.bg }}>
                            {cfg.label}
                          </span>
                        </Tooltip>
                      ) : (
...
const WashoutMatrix = () => {
  ...
  if (isLoading)
    return <div className="flex justify-center items-center py-24"><Spin size="large" tip="Loading Washout Matrix..." /></div>;

  if (error)
    return <div className="p-10"><Empty description="Error loading Washout Matrix data" /></div>;
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="m-0 text-base font-black text-[#002060] uppercase tracking-tight">Washout Matrix</h2>
            <p className="m-0 text-xs text-slate-400 font-medium mt-0.5">Source to Target GCAS washout rules</p>
          </div>
        </div>
        <Legend />
      </div>
      <div className="px-6 py-5">
        <Tabs
          ...
          items={MATRIX_TABS.map(t => ({
            ...
            children: <div className="pt-4"><MatrixGrid rows={matrices[t.key] || []} /></div>,
          }))}
        />
      </div>
    </div>
